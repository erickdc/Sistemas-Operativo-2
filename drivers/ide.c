    ...
    byte_t mask, byte_t value, unsigned long timeout)
{
    byte_t status;
    do {
        status = port_byte_in(controller->iobase + ATA_STATUS);
...
static bool_t reset_controller(struct ide_controller *controller)
{
    ...
    port_byte_out(iobase + ATA_DEV_CTL, ATA_CTL_SRST); mdelay(2);
    ...
    port_byte_out(iobase + ATA_DEV_CTL, 0);
...
static bool_t select_device(struct ide_device *device)
{
    ...
    if ((port_byte_in(iobase + ATA_STATUS) & (ATA_STATUS_BSY | ATA_STATUS_DRQ)))
        return FALSE;

    /* Select the drive. The spec says to wait at least 400 ns before
       reading the status register to ensure its content is valid. */
    port_byte_out(iobase + ATA_DRV_HEAD, 0xa0 | (device->position << 4)); udelay(1);
...
static void identify_ide_device(struct ide_device *device)
{
    ...
    port_byte_out(iobase + ATA_NSECTOR, 0xab);
    if (port_byte_in(iobase + ATA_NSECTOR) != 0xab)
        return;
    ...
    if (port_byte_in(iobase + ATA_NSECTOR) == 0x01 &&
        port_byte_in(iobase + ATA_SECTOR) == 0x01) {
        cl = port_byte_in(iobase + ATA_LCYL);
        ch = port_byte_in(iobase + ATA_HCYL);
        status = port_byte_in(iobase + ATA_STATUS);
        if (cl == 0x14 && ch == 0xeb) {
    ...
    port_byte_out(iobase + ATA_COMMAND, cmd); udelay(1);
    ...
    for (i = 0; i < 256; i++)
        info[i] = port_word_in(iobase + ATA_DATA);
    ...
    unsigned int nblocks, void *buffer, int type)
{
    ...
    port_byte_out(iobase + ATA_NSECTOR, nblocks);
    port_byte_out(iobase + ATA_SECTOR, sc);
    port_byte_out(iobase + ATA_LCYL, cl);
    port_byte_out(iobase + ATA_HCYL, ch);
    port_byte_out(iobase + ATA_DRV_HEAD, (device->lba << 6) | (device->position << 4) | hd);
    port_byte_out(iobase + ATA_COMMAND, cmd);
    ...
    if (port_byte_in(iobase + ATA_STATUS) & ATA_STATUS_ERR) {
        return 0;
    }
    ...
    if (type == IO_WRITE) {
        ...
        for (i = nblocks * 256; --i >= 0; )
            port_word_out(iobase + ATA_DATA, *buf++);
    }
    ...
    if (type == IO_READ) {
        ...
        for (i = nblocks * 256; --i >= 0; )
            *buf++ = port_word_in(iobase + ATA_DATA);
    }
 ...
 */
unsigned int ide_read_blocks(unsigned int minor, offset_t block,
 ...
 */
unsigned int ide_write_blocks(unsigned int minor, offset_t block,
    unsigned int nblocks, void *buffer)
{
...
static void handle_primary_ide_controller_interrupt(struct regs *regs)
{
    handle_ide_controller_interrupt(regs->esp, &controllers[PRIMARY_IDE_CONTROLLER]);
}

static void handle_secondary_ide_controller_interrupt(struct regs *regs)
{
    handle_ide_controller_interrupt(regs->esp, &controllers[SECONDARY_IDE_CONTROLLER]);
}
...
void init_ide_devices(void)
{
    ...
    for (i = 0; i < NR_IDE_CONTROLLERS; i++) {
        ...
        for (j = 0; j < NR_DEVICES_PER_CONTROLLER; j++) {
            ...
            /*kprintf("Probing device %d %d '%s' '%s' \n", i, j,
                    device->present? "present" : "not present",
                    device->atapi? "atapi" : "not atapi"
                    );*/
            ...
            kprintf("%s [%u-%u]: %s (%u/%u/%u - %u sectors) LBA:%s - DMA:%s\n",
    ...
    irq_install_handler(IRQ_PRIMARY_IDE, handle_primary_ide_controller_interrupt);
    irq_install_handler(IRQ_SECONDARY_IDE, handle_secondary_ide_controller_interrupt);
